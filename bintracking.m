function [R4,im_color,bin_seq] = bintracking(im2_b,im_c,R1,R4,region_pos2,bin_seq)
r1_obj = R1.r1_obj;
r1 = R1.r1;
r4_obj = R4.r4_obj;
r4_cnt = R4.r4_cnt;
...
threshold = 145;%threshold for object recognition
dis_enter = 230;%enter distance (coordinate x)
dis_enter_y = 80;%enter distance (coordinate y)
dis_exit = 255;%exit distance (coordinate x)
%% Preprocessing
    ...
    if (r4_cnt ~= 0)
            ...
            if (temp_r4(i,4)==0)
                if (temp_r4(i,2)<dis_enter && temp_r4(i,1)<dis_enter_y)
                    ...
                    if min(dis1)>=50
                        ...
                        py2=min(region_pos2(4),round(temp_r4(i,2)+35));
                        ...
                        dist_b_p=pdist2(r1_edge,temp_r4(i,1:2)++[r4(1) r4(3)],'euclidean');
        ...
        for q = 1:pcnt_r4
            if (temp_r4(q,2)<dis_enter && temp_r4(q,1)<dis_enter_y && temp_r4(q,3)>limit_area)
                ...
                r1_edge(:,1)=r1_edge(:,1)-sqrt(r1_obj(:,3));
                r1_edge(r1_edge(:,1)<r1(1),1)=r1(1);
                dist_b_p=pdist2(r1_edge,temp_r4(q,1:2)++[r4(1) r4(3)],'euclidean');
                dist_b_p_min=min(dist_b_p);
                if dist_b_p_min>max_dist_b_p
                    continue;
                end
