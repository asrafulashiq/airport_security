if R_9.current_frame <= R_9.end_frame
    ...
    % flow image
    try
        im_flow_all_box = imread(fullfile(R_9.flow_dir, sprintf('%04d_flow.jpg', R_9.current_frame)));
        im_flow_all = imrotate(im_flow_all_box, R_9.rot_angle);
        if detect_bad_flow(im_flow_all, R_9.max_flow_thres) == false
            im_flow_all = [];
        end
    catch
    ...
    if ~isempty(im_flow_all)
        ...
        [R_9.R_people, R_2.R_people] = people_detector_tracking(im_r, im_flow_people, ...
            R_9.R_people, R_2.R_people);
    else
        im_flow_people = [];
        disp('BAD FLOW : Camera 9');
    end
