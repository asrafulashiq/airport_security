function [R_people, R_people_9] = people_detector_tracking_camera_2(im_r, im_flow, R_people, R_people_9)
...
if ~isempty(R_people.people_array) && ~isempty(list_bbox)
    ...
    for i = 1:numel(R_people.people_array)
        ...
        if person.Centroid(1) < R_people.enter_x_cam_9 && ...
                person.Centroid(2) <  R_people.enter_y_cam_9 && ...
                a > 180 && a < 360
            R_people.people_array{i}.state = "goto9";
            del_exit(end+1) = i;
            R_people_9.stack_of_people{end+1} = R_people.people_array{i};
        end
...
for i = 1:numel(body_prop)
    ...
    if body_prop(i).Centroid(1) < R_people.limit_init_x && ...
            ...
            %body_prop(i).Centroid(1) > R_people.exit_x_cam_9_all_min
%         if R_people.associate_9
%         k = 0;
%         if numel(R_people.stack_of_people) > 0 && body_prop(i).Centroid(1) > R_people.exit_x_cam_9_min && ...
%            abs(R_people.stack_of_people{end}.Centroid(2) + 60 - body_prop(i).Centroid(2)) < 30
%             k = 1;
%         elseif 
%             
%         end
...
%         end
        ...
        min_val_ind = [1000 -1]; % [distance index]
