function [R_people, R9_people] = people_detector_tracking_camera_2(im_r, im_flow, R_people, R9_people)
            ...
            for i = 1:length(vect)
                if count_el(i) == 1
                    ...
                    if isinf(min_dis_vector(prev_index, 1)) || ...
                            ~ ( body_prop(min_arg).Area < 5*R_people.people_array{prev_index_p}.Area) %&& ...
...
%% initial detection
if numel(R_people.stack_of_people) > 0
    ...
    for i = 1:numel(body_prop)
        ...
        if body_prop(i).Centroid(1) < R_people.limit_init_x && ...
                ...
                body_prop(i).Centroid(1) > R_people.exit_x_cam_9_all
            ...
            min_val_ind = [600 -1]; % [distance index]
            for j=1:numel(R_people.stack_of_people)
                  % approximately add 40 to convert from c9 to c2
                  search_y = R_people.stack_of_people{j}.Centroid(2) + 40; 
                  if abs(search_y - body_prop(i).Centroid(2)) < min_val_ind(1)
                      min_val_ind = [abs(search_y - body_prop(i).Centroid(2)) j];
                  end
            end
            if min_val_ind(2) ~= -1
               Person = R_people.stack_of_people{min_val_ind(2)};
               Person.Centroid = body_prop(i).Centroid;
               Person.BoundingBox = body_prop(i).BoundingBox;
               
               [a, m] = calcAngleMag(im_flow_hsv, im_binary, body_prop(i).BoundingBox);
               
               Person.flow_angle = a; Person.flow_mag = m;
               
               R_people.people_array{end+1} = Person;
               R_people.stack_of_people(min_val_ind(2)) = [];
               
               for j=1:numel(R9_people.people_array)
...
del_tmp = [];
for j =1 : numel(R_people.stack_of_people) 
    R_people.stack_of_people{j}.temp_count = R_people.stack_of_people{j}.temp_count + 1;
    if R_people.stack_of_people{j}.temp_count > 30
       del_tmp(end+1) = j; 
    end
end
R_people.stack_of_people(del_tmp) = [];
